...
2. Đọc ĐÚNG MỘT file `../dym-setup-<slug>/SKILL.md` — đường **tương đối từ thư mục chứa chính file hub này** (thư mục skill), nên chạy đúng dù bundle nằm ở `doyourmagic/setup/skills/` hay đã cài qua `npx skills add rheinmir/dym` vào `.claude/skills/`. Rồi làm theo `## Steps` và `## Rules` của nó. Không đọc file con khác.
